...
static gboolean
buoh_view_load_comic (gpointer gdata)
{
	BuohView         *view = BUOH_VIEW (gdata);
...
static gboolean
buoh_view_check_comic_uri (BuohView *view)
{
	GnomeVFSURI *vfs_uri;
	gchar       *uri;
	gboolean     result;
	...
	result = gnome_vfs_uri_exists (vfs_uri);
		...
		if (view->priv->comic) {
			...
			if (!pixbuf) {
				if (buoh_view_check_comic_uri (view)) {
					g_idle_add (buoh_view_load_comic, (gpointer) view);
				} else {
					if (view->priv->message) {
						g_free (view->priv->message);
					}
